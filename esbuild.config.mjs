	...
	setup(build) {
		build.onEnd(async () => {
			try {
				...
				await mkdir('test-vault-myplugin/.obsidian/plugins/my-plugin', { recursive: true });
				// Copy manifest.json
				await copyFile('manifest.json', 'test-vault-myplugin/.obsidian/plugins/my-plugin/manifest.json');
				// Copy all content from mockOb directory
				await copyDir('mockOb', 'test-vault-myplugin');
