	...
	setup(build) {
		build.onEnd(async () => {
			try {
				...
				await copyDir('mockOb', `${vaultPath}/.obsidian/plugins/my-plugin`);
...
const copyPlugin2 = {
	...
	setup(build) {
		build.onEnd(async () => {
			const vaultPath = '~/obsidian-note/obsidian-personal/.obsidian/plugins/my-plugin';
			try {
				// Ensure plugin directory exists
				await mkdir(vaultPath, { recursive: true });
				// Copy manifest.json
				await copyFile('manifest.json', `${vaultPath}/manifest.json`);
				console.log('✓ Plugin files copied successfully into', vaultPath);
			} catch (error) {
				console.error('Error copying plugin files:', error);
				process.exit(1);
			}
